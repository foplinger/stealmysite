...
const headerCharacter = 'h';
const githubCharacter = 'g';
...
const generateTabBody = (content) => {
    if (content) {
        return Object.keys(content).map(snippet => {
            ...
            } else if (snippet.split(splitCharacter)[0] === headerCharacter) {
                return (
                    <Typography variant="subtitle2" gutterBottom>
                        {content[snippet]}
                    </Typography>
                )
            } else if (snippet.split(splitCharacter)[0] === githubCharacter) {
                return (
                    <div style={{width: '100%', display: 'flex', justifyContent: 'center'}}>
                        <IconButton  className={'githubLink'} onClick={() => handlePageChange(content[snippet])}>
...
const getTabContentWithInnerTabs = (tabConfig) => {
    

    return (
        <InnerTab tabConfig={tabConfig}/> 
    )
    
}

export const getTabContentWithoutInnerTabs = (tabConfig) => {
    return (
        <div style={{marginTop: '30px'}}>
            <Typography variant="h6" gutterBottom>
                {tabConfig && tabConfig.title}
            </Typography>
            <div>
                {generateTabBody(tabConfig.content)}
            </div>
